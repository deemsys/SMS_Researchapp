...
public class BroadCastController {
	...
	public String insertsendstream(
			@ModelAttribute("broadCast") @Valid BroadCast broadCast,
			BindingResult result, ModelMap model) {
//		List<ParticipantsDetails> parts=mainDAO.getParticipantsbyGroupName(broadCast.getGroup_id());
		
		HashMap<String, ArrayList<ParticipantsDetails>>   map= mainDAO.getAllParticipants();
		
		String grname= participantGroupDAO.getGroupName(broadCast.getGroup_id());
		ArrayList<ParticipantsDetails>  list=map.get(grname);
		
		 

		int status = broadDAO.insertNewBroadCast(broadCast);
		for (ParticipantsDetails participantsDetails : list) {
			try {
				messageSender.sendSMS(participantsDetails.getMobile_num(), "Deemsys test");
